...
public class AuditPdf {
	...
	protected static final String AUDIT_SUFFIX = "_Audit";
	protected static final String REPLAY_SUFFIX = "_Audit_Replay";
	protected static final String COVER_SUFFIX = "_CoverPage";
	...
	public AuditPdf() {
		formats = new HashMap<>();
		formats.put(PDFDataProvider.HEADING1, new Format(Color.RED, PDType1Font.HELVETICA_BOLD, 12f));
		formats.put(PDFDataProvider.HEADING2,
				new Format(Color.decode(BROWN_COLOR_CODE), PDType1Font.HELVETICA_BOLD, 12f));
		formats.put(PDFDataProvider.DISCLAIMER1, new Format(Color.RED, PDType1Font.COURIER_BOLD, 10f));
		formats.put(PDFDataProvider.DISCLAIMER2,
				new Format(Color.decode(BROWN_COLOR_CODE), PDType1Font.COURIER_BOLD, 10f));
		formats.put(PDFDataProvider.URL, new Format(Color.BLUE, PDType1Font.HELVETICA_BOLD, 12f));
		formats.put(PLAIN_FORMAT, new Format(Color.BLACK, PDMMType1Font.COURIER_BOLD, 10f));

	}

	public File createAuditPdf(@NonNull final String s) {
		List<String> resultList = new ArrayList<>();
		if (resultList.isEmpty())
			return null;
		return createAuditFile(s, resultList, AUDIT_SUFFIX);
	}
	...
	private File createAuditFile(final String auditData, final List<String> reportData, final String suffix) {
		File result = null;
		try {
			document = newDocument();
			populatePages(reportData);
			result = writeToFile(null, suffix);
		} catch (Exception e) {
	...
	private File writeToFile(final String docId, final String suffix) throws IOException {
		...
		document.save(file);
		return mergeWith(file, docId, suffix);
	}

	private File mergeWith(final File file, final String docId, final String suffix) {
		File original = getReportFile(docId, COVER_SUFFIX.equals(suffix) ? "" : suffix);
	...
	private File getFile(final String docId, final String suffix) {
		File result = getReportFile(docId, suffix);
		if (result.exists() && !COVER_SUFFIX.equals(suffix))
			result = getReportFile(docId, REPLAY_SUFFIX);
		return result;
	}
	...
	private String getUnformattedText(final String line) {
		return line.contains(PDFDataProvider.DIVIDER) ? StringUtils.substringAfter(line, PDFDataProvider.DIVIDER)
				: line;
	}
	...
	private void writeUrl(final String line) {
		...
		String[] tokens = line.split("\\" + PDFDataProvider.DIVIDER, 3);
		newLine();
	...
	private Format getFormat(String text) {
		String format = text.startsWith(PDFDataProvider.FORMAT)
				? StringUtils.substringBetween(text, PDFDataProvider.FORMAT, PDFDataProvider.DIVIDER)
				: text.startsWith(PDFDataProvider.URL) ? PDFDataProvider.URL : PLAIN_FORMAT;
	...
	private boolean isUrlText(String line) {
		return StringUtils.startsWith(line, PDFDataProvider.URL);
	}
