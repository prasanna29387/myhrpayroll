 ...
 * Requires the precision to be specified for double to Money conversion.
 * Precision is usually based on the smallest tick in your exchange data.
 */
public class MoneyFactory {
	...
	protected static final long[] MULTIPLIERS = new long[MoneyFactory.MAX_ALLOWED_PRECISION + 1];
	/** Non-positive powers of 10 */
	protected static final double[] MULTIPLIERS_NEG = new double[MoneyFactory.MAX_ALLOWED_PRECISION + 1];
	 ...
	 * 
	 * @param units
	 *            Currency units (cents, for example)
	 * @param precision
	 *            Number of digits after decimal point in your smallest possible
	 *            currency unit. Should be between 0 and
	 *            <code>MAX_ALLOWED_PRECISION</code> (inclusive).
	 * @return Money object
	 * @throws IllegalArgumentException
	 *             In case of invalid precision
	 */
	...
	/**
	 * Same as <code>fromString</code>, but characters are extracted from the
	 * given part of char array
	 *
	 * @param chars
	 *            Char array
	 * @param offset
	...
	/**
	 * Same as <code>fromString</code>, but characters are extracted from the
	 * given part of byte array. We expect the data to be ASCII-encoded in the
	 ...
	 *
	 * @param seq
	 *            <code>CharSequence</code> object, may be more convenient in
	 *            some situations.
	 * @return Money object
	 */
	...
	/**
	 * Convert a String monetary value into a Money object. We support a single
	 * format - dot-separated decimal part without the scientific notation
	 * support ( valid example - 355.56 ).
	 *
	 * @param value
	 *            Value to parse
	 * @return Money
	...
	private static Money fromString0(final String value) {
		...
		try {
			...
			return new MoneyLong(units, precision); // actual precision, not the
													// maximal one
	...
	/**
	 * Fast single pass parser, should correctly process most of
	 * positive/negative values fitting into <code>long</code>
	 *
	 * @param str
	 *            Floating point number to parse
	 * @return Money object or null (if can't parse)
	 * @throws IllegalArgumentException
	 *             If a value has more than one decimal digit
	 */
			...
			if (c == '.') {
				if (precision > 0)
					throw new IllegalArgumentException(
							"Unparseable String value has more than 1 decimal point: " + str);
	...
	/**
	 * <p>
	 * Convert a double monetary value into a Money object. You will end up with
	 * the most efficient Money type if you have no more than
	 * <code>MAX_ALLOWED_PRECISION</code> decimal digits in your value.
	 * </p>
	 * <p>
	 * This method will attempt to look by ulp in both directions during
	 * conversions to cater for already slightly incorrect values - results of
	 * <code>double</code> operations outside this library.
	 * </p>
	 * 
	 * @param value
	 *            Double monetary value
	 * @return Money object
	 */
	 ...
	 * Convert a double monetary value into a Money object. You will end up with
	 * the most efficient Money type if you have no more than
	 * <code>precision</code> decimal digits in your value.
	 * </p>
	 * <p>
	 * This method will attempt to look by ulp in both directions during
	 * conversions to cater for already slightly incorrect values - results of
	 ...
	 * </p>
	 * <p>
	 * Do not try to set too high precision for this method - it may prevent you
	 * from correcting a slightly incorrect value (off by ulp) into a correct
	 * one. As a result, you will end up with BigDecimal-based implementation,
	 * which requires more memory and which is much slower to calculate.
	 * </p>
	 * 
	 * @param value
	 *            Double monetary value
	 ...
	 *            Number of digits after decimal point in your smallest possible
	 *            currency unit. Should be between 0 and
	 *            <code>MAX_ALLOWED_PRECISION</code> (inclusive). This parameter
	 *            is a hint only for more efficient conversion. It does not
	 *            truncate the result.
	 * @return Money object with a value as close as possible to a provided
	...
	private static MoneyLong fromDouble0(final double value, final int precision) {
		...
		if (multiplied == converted) // here is an implicit conversion from long
										// to double
	...
	/**
	 * <p>
	 * Convert a given BigDecimal value into money. Conversion is similar to
	 * <code>toDouble</code>, though this method does not attempt to make any
	 * corrections: it assumes that BigDecimal is a result of exact
	 * calculations.
	 * </p>
	 * <p>
	 * This method will try to use the most efficient representation if
	 * possible.
	 * </p>
	 * 
	 * @param value
	 *            BigDecimal value to convert
	 * @return Money object
	 */
	public static Money fromBigDecimal(final BigDecimal value) {
		final BigDecimal cleaned = value.stripTrailingZeros();

		// try to convert to double using a fixed precision = 3, which will
		// cover most of currencies
