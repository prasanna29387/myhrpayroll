...
/**
 * <p>
 * An abstraction for a money value using as efficient as possible format to
 * keep and process your data. All methods allocating a new Money object will
 * try to output the most efficient representation (for example, if precision=1,
 * then 0.05+0.05 (each of them does not fit into precision) will be equal to
 * 0.1 (units=1, precision=1).
 * </p>
 * <p>
 * You should use one of <code>to*</code> methods to convert it into a more
 * commonly used formats after the calculation is over.
 * </p>
 */
public interface Money extends Comparable<Money>, Serializable {
	...
	/**
	 * Compares another Money object to this one. Should be used like
	 * {@link BigDecimal#compareTo(Object)}. May or may not share the same
	 * specification, read return. However this method will be up to
	 * specification as defined in {@link Comparable}
	 * <p>
	 * Two {@code Money} objects that are equal in value but have a different
	 ...
	 * {@code 0)}, where {@code op} is one of the six comparison operators.
	 * 
	 * @param other
	 *            Other money object
	 * @return As of this version, -1, 0, or 1 as this {@code Money} is
	 *         numerically less than, equal to, or greater than {@code other}.
	 *         This specification may possibly change to -x or x with x
	 *         representing the difference in precision. I.E If the number is
	 *         negative than it will still be numerically less than
	 *         {@code other}.
	 */
	@Override
